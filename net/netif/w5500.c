...
{ WORD x, y = w5_r16(bsb, a); int i = 0; do { x = y; y = w5_r16(bsb, a); } while (x != y && ++i < 4); return y; }
...
static int w5_try(int bus, int cs)
{
    ...
    g_bus = bus; g_cs = cs;
    if (w5_r1(BSB_COMMON, VERSIONR) == 0x04) return 1;   // chip signature
...
int W5500Init(unsigned char mac[6])
{
    int t;
    if (g_bus < 0) return 0;
    w5_w1(BSB_COMMON, MR, 0x80);                          // soft reset
    for (t = 0; t < 1000 && (w5_r1(BSB_COMMON, MR) & 0x80); t++) ;
    w5_write(BSB_COMMON, SHAR, g_w5mac, 6);               // our MAC
    w5_w1(BSB_S0_REG, Sn_RXBUF, 16);                      // socket 0 gets the whole 16K
    ...
    if (w5_r1(BSB_S0_REG, Sn_SR) != SOCK_MACRAW) return 0;
    memcpy(mac, g_w5mac, 6);
    { WCHAR b[64]; wsprintfW(b, L"w5500: up on bus %d MAC=%02x:%02x:%02x:%02x:%02x:%02x\r\n",
        g_bus, g_w5mac[0],g_w5mac[1],g_w5mac[2],g_w5mac[3],g_w5mac[4],g_w5mac[5]); OutputDebugStringW(b); }
    return 1;
}
